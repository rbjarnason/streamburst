...
class CheckoutController < ApplicationController
  ...
  def payment
    ...
    if @brand.id==18 and @currency_code=="ISK"
      @card_type_label = ["Visa", "MasterCard", "PayPal","Google"]
      @card_type_data = ["visa", "master", "paypal","google"]
    else
...
#     :number => '4111111111111111', #Visa paypal pro test
  ...
  def isk_payment
    ...
    if @order.currency_code == "ISK" and @brand.id == 18      
      exp_yr = params[:card][:expiry_year][2..3]
      exp_mo = "%02d" % params[:card][:expiry_month].to_s
    ...
    if res_code.to_i == 202 or res_code.to_i == 200
  ...
  def grapewire_payment(card, amount, expiration, cvc, refid, email)
    ...
    url = URI.parse("https://rest.grapewire.net/creditcard")
